...
export default function AnalyzeForm({ onAnalyze, initialPreset = null }) {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(async () => {
      // onAnalyze ahora es async (consulta /api/catalog-lookup, ver Fase 1
      // de migración del catálogo): esperamos a que termine antes de quitar
      // el spinner, para que "Consultando histórico..." sea real y no un
      // temporizador vacío.
      try {
        ...
        if (matchingPreset && matchingPreset.inputPrice === parseFloat(price)) {
          await onAnalyze(matchingPreset);
        } else {
          await onAnalyze({
