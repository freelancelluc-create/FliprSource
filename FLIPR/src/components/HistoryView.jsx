...
export default function HistoryView({ items = [], mode = "history", onSelectResult, onDeleteItem }) {
  ...
  const icon = mode === "history" ? <History className="w-6 h-6 text-emerald-700" /> : <Bookmark className="w-6 h-6 text-amber-700" />;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 space-y-8">
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-slate-200 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
          ...
          <div className="flex items-center gap-3 font-mono">
            <div className="rounded-2xl bg-slate-50 border border-slate-200 p-3 text-right">
              <span className="text-[10px] text-slate-600 uppercase block">Oportunidades CÓMPRALO</span>
              <span className="text-lg font-black text-emerald-700">{buyCount} oport.</span>
            </div>

            <div className="rounded-2xl bg-slate-50 border border-slate-200 p-3 text-right">
              <span className="text-[10px] text-slate-600 uppercase block">Beneficio Potencial</span>
              <span className="text-lg font-black text-emerald-700">+{totalPotentialProfit} €</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-slate-600">Filtrar:</span>
          </div>
          ...
          <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-xl border border-slate-200 text-xs">
            <button
              ...
              className={`px-3 py-1 rounded-lg font-bold transition-all ${
                filter === "ALL" ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              Todos ({items.length})
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg font-bold transition-all ${
                filter === "COMPRALO" ? 'bg-emerald-700 text-white' : 'text-emerald-700 hover:bg-emerald-500/10'
              }`}
            >
              CÓMPRALO
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg font-bold transition-all ${
                filter === "NEGOCIA" ? 'bg-amber-500 text-black' : 'text-amber-700 hover:bg-amber-500/10'
              }`}
            >
              NEGOCIA
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg font-bold transition-all ${
                filter === "PASA" ? 'bg-red-500 text-white' : 'text-red-600 hover:bg-red-500/10'
              }`}
            >
              PASA
            </button>
      ...
      {filteredItems.length === 0 ? (
        <div className="glass-panel rounded-3xl p-12 text-center space-y-4 border border-slate-200">
          <Zap className="w-12 h-12 text-slate-500 mx-auto" />
          <h3 className="text-lg font-bold text-slate-900">No hay análisis registrados</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredItems.map((item) => (
            <div
              key={item.id}
              className="glass-panel-interactive rounded-2xl p-5 border border-slate-200 space-y-4 relative group"
            >
              {/* Top row */}
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-slate-600 font-mono">
                      Compra: <strong className="text-slate-900">{item.inputPrice} €</strong>
                    </span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="text-slate-500 hover:text-red-600 p-1 opacity-60 group-hover:opacity-100 transition-opacity"
              ...
              <div className="flex items-center justify-between border-t border-slate-200 pt-3">
                <span className={`px-3 py-1 rounded-xl text-xs font-mono font-black ${
                  item.verdict === 'COMPRALO' ? 'bg-emerald-700 text-white' :
                ...
                <div className="flex items-center gap-2 font-mono text-xs">
                  <span className="text-slate-600">FLIP SCORE:</span>
                  <span className="font-bold text-emerald-700">{item.flipScore}/100</span>
                </div>
              </div>
